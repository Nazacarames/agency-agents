    ...
    def _collab_block(self) -> str:
        ...
        try:
            ...
            parts.append(
                ...
                "- `crm-web` → cambio en la landing del CRM (crm.automiq.agency). Es otro "
                "repo y otro deploy: web_optimizer NO puede tocarlo. Va al dueño igual que "
                "`dev`, pero separado para no mezclarlo con el código de los agentes.\n"
                ...
                "  ⚠️ **Si además hace falta un DATO o un ACCESO que sólo tiene el dueño "
                "(un ID de Pixel, una credencial, entrar a una cuenta), va a `humano`, NO "
                "a `dev`** — aunque después haya que tocar código. Sin ese dato no se puede "
                "escribir una línea, así que en `dev` es trabajo imposible que además tapa "
                "lo que sí se puede hacer. El 2026-09-08 había 5 ítems así parados en `dev`, "
                "4 de ellos pidiendo los mismos dos IDs.\n"
                ...
                "El backlog es de NUESTRA operación. Lo que encontraste auditando el sitio "
                "de un lead o de un cliente va en el entregable de esa auditoría, NUNCA como "
                "`PENDIENTE`: nadie acá puede tocar el sitio de un tercero, y el ítem termina "
                "pidiéndole al dueño datos que no tiene. (El 2026-08-15 entró 'cambiar el H1 "
                "TU ALIADO EN CADA VENTA' — el H1 de un prospecto — como tarea de la landing.)\n"
                "El título va en UNA línea corta (~200 caracteres): qué hay que hacer. El "
                "porqué y el detalle van en tu reporte, no en el título.\n"
